...
(function(){
  ...
  theme.CartForm = (function() {
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      cartMarkup: function(text) {
        ...
        var drawerSummaryMoneyFormat = '{{amount_with_comma_separator}} €';
        if (summarySubtotal) {
          summarySubtotal.innerHTML = theme.Currency.formatMoney(subtotal, drawerSummaryMoneyFormat);
        }
        if (summaryDiscount) {
          summaryDiscount.innerHTML = '−' + theme.Currency.formatMoney(discount, drawerSummaryMoneyFormat);
        }
        this.subtotal.innerHTML = theme.Currency.formatMoney(total, drawerSummaryMoneyFormat);
      ...
      updateCount: function(count) {
        ...
        var countLabels = document.querySelectorAll('[data-cart-count-label]');
        if (countLabels.length) {
          countLabels.forEach(el => {
            el.setAttribute('aria-label', `Warenkorb (${count})`);
          });
        }
  ...
  theme.QtySelector = (function() {
    ...
    function QtySelector(el, options) {
      ...
      this.quantityMultiplier = parseInt(this.input.dataset.quantityMultiplier, 10) || 1;
      ...
      _change: function(qty) {
        ...
        if (this.options.isCart) {
          document.dispatchEvent(new CustomEvent('cart:quantity' + this.options.namespace, {
              detail: [this.options.key, qty * this.quantityMultiplier, this.wrapper]
          }));
        }
      }
    });
  ...
  theme.headerNav = (function() {
    ...
    function init() {
      ...
      const usesNativeSticky = siteHeader.dataset.nativeSticky === 'true' && siteHeader.closest('.site-header-group');
      if (usesNativeSticky) {
        // The header group owns the native sticky positioning. Do not switch
        // the header to fixed positioning after a scroll threshold.
        theme.config.stickyHeader = true;
      } else if (config.stickyEnabled) {
      ...
      openModalProduct: function() {
        ...
        if (!this.settings.modalInit) {
          ...
          fetch(url).then(function(response) {
            ...
            // Populate window.inventories before formSetup() so the on-load
            // updateInventory() call can read this product's inventory data.
